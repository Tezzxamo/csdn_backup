...
```text
CAS: compare and set，核心思想是比较。

如果预期值（A）与内存值（V）相等，则用新值（B）更新内存值。
即：if(A == V) {
    V = B;
   } else {
     // do nothing;
...
```text
ReentrantLock：是一种可重入锁，基于AQS实现。

AQS：
  abstract queued synchronizer，抽象的队列式同步器。
  核心思想：共享资源如果空闲，将请求资源的线程设置为有效的工作线程，并将共享资源设为锁定状态；
           如果共享资源被占用，则将请求线程放入CLH队列中。
  CLH队列：虚拟的双向队列，没有队列实例，仅存在节点之间的关联关系，将每个请求线程封装成一个节点。
  AQS有两种资源共享方式：独占（如ReentrantLock);共享（如CountDownLatch）
  它维护了一个 volatile int state 变量，对于ReentrantLock，它是资源占有情况；对于CountDownLatch，它是当前计数器的值。
```
